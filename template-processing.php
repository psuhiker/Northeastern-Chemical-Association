<body class="secondary processing">
    ...
    <?php if (false !== strpos($url,'?testing123' )) { // Testing ?>
	...
	<?php } elseif ( is_page('474') ) { // Membership Renewal ?>
	    ...
	    <?php if (false !== strpos($url,'?update' )) { ?>
	    	...
	    	<div class="inner">
	    		...
	    		<p><img src="<?php bloginfo('template_directory'); ?>/images/loading.gif" width="100">

				<?php
				    global $current_user; get_currentuserinfo();
				    $member_id = get_current_user_id();
				    $currentExpire = get_field('member_renewal', 'user_' . $member_id . '', false, false);
				    //$currentExpire = new DateTime($currentExpire);
				    //$currentExpireYear = $currentExpire->format('Y');
				    //$nextExpireYear = $currentExpireYear;
				    //$nextExpireYear++;
				    //$nextExpire = $nextExpireYear . '1231';
				    $currentYear = date(Y) . '1231';
		    		$newYear = date(Y) + 1;
		    		$currentDay = date(z) + 1;
		    		$nextYearTrigger = 335;
		    		if ($currentDay > $nextYearTrigger) {
		    			if ($currentExpire < $currentYear) {
		    				$nextExpire = $newYear . '1231';
		    			} else {
		    				$nextExpire = $currentExpire + 10000;
		    			}
		    		} else {
		    			if ($currentExpire < $currentYear) {
		    				$nextExpire = $currentYear;
		    			} else {
		    				$nextExpire = $currentExpire + 10000;
		    			}
		    		}
					update_user_meta( $member_id, 'member_renewal', $nextExpire );
				?>
